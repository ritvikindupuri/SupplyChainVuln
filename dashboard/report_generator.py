...
Within \"Session Overview\", you MUST include a clearly labeled subsection (### level) called:
### Target
This subsection MUST include:
- The target URL written exactly and prominently
- A brief 2-4 sentence description of what the target is (e.g., custom internal web application, local dev app, staging environment) and why it is in-scope
...
The TARGET URL is: {target_url if target_url else "Not specified"}. This URL was provided by the user and represents the web application that was being analyzed during this penetration testing session. All traffic, alerts, and findings in this report pertain to this target.
...
Do NOT wrap the response in a code block. Output plain markdown only. Every section MUST be excruciatingly detailed -- write at least 5-8 substantial paragraphs per section."""
    ...
    def _fallback_report(self, severity_counts, top_alerts, target_url="", error=None):
        ...
        if target_url:
            ...
            lines.append("")
            lines.append("The target in this assessment is the custom web application endpoint configured by the operator via the PacketSentry dashboard. The scope is limited to traffic to/from this target and the associated application behavior observed during the monitoring window.")
            lines.append("")
    ...
    def _build_pdf(self, pdf_path, report_md, alert_count, packet_count, activity_count, severity_counts, top_alerts, target_url=""):
        ...
        def ensure_space(needed_mm):
            # Prevent section headers and other key lines from being cut off at the page bottom.
            # FPDF will auto-break, but we want to avoid placing headers near the break line.
            if pdf.get_y() + needed_mm > (297 - 22):
                pdf.add_page()
